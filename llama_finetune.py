...
def train(model_dir, batch_size, training_steps, stage, output_dir, *args):
    ...
    if not os.path.exists(model_quant_dir):
        ...
        def process(x):
            ...
            x["input_ids"] = torch.clamp(torch.squeeze(input_ids, -2), int(input_ids.min()), model.model.embed_tokens.num_embeddings - 1)
            x["labels"] = torch.clamp(torch.squeeze(labels, -2), int(labels.min()), model.model.embed_tokens.num_embeddings - 1)
            return x
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-dir", type=str, default='/mnt/sgnfsdata/tolo-03-97/pretrained_models/llama2-7b-chat-hf')#change internlm2-->llama2 model, cannot run
    parser.add_argument("--batch-size", type=int, default=2)
    parser.add_argument("--training-steps", type=int, default=1) #100
    parser.add_argument("--stage", type=int, default=0)
    parser.add_argument("--output-dir", type=str, default='Llama2_quantized_saved_finetune_model/')
    #parser.add_argument("--tmplate", type=str, default='llama2_chat') #PROMPT_TEMPLATE.llama2_chat
